...
async function mkBlueprint(input: string, output: string) {
  const plutusJson: Blueprint = JSON.parse(fs.readFileSync(input, "utf8"));
  const plutusVersion =
  ...
  fs.writeFileSync(output, plutus, "utf8");
...
const imports = `// Autogenerated from mkBlueprint
import {Data, Validator, applyParamsToScript} from "@lucid-evolution/lucid";
...
function mkValidator(
  ...
  validator: {
    ...
    datum: any;
    redeemer: any;
    parameters: { schema: any; title: string }[];
    compiledCode: any;
  },
) {
  const title = validator.title;
  const name = title.split(".").map(snakeToCamel).map(upperFirst).join("");
  if (name.endsWith("Else")) return;
  ...
  const redeemerTitle = snakeToCamel(redeemer.title || "") || "_redeemer";
